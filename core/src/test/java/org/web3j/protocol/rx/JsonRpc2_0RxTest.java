...
public class JsonRpc2_0RxTest {
    ...
    public void testReplayBlocksFlowable() throws Exception {
        ...
        Disposable subscription = flowable.subscribe(
                new Consumer<EthBlock>() {
                    @Override
                    public void accept(EthBlock result) throws Exception {
                        results.add(result);
    ...
    public void testReplayBlocksDescendingFlowable() throws Exception {
        ...
        final List<EthBlock> results = new ArrayList<>(ethBlocks.size());
        Disposable subscription = flowable.subscribe(
                new Consumer<EthBlock>() {
                    @Override
                    public void accept(EthBlock result) throws Exception {
    ...
    public void testReplayTransactionsFlowable() throws Exception {
        ...
        for (EthBlock ethBlock : ethBlocks) {
            ...
            for (EthBlock.TransactionResult transactionResult : transactionResults) {
                expectedTransactions.add((Transaction) transactionResult.get());
            }
        }
